...
describe 'apache::vhost', :type => :define do
  ...
  describe 'os-independent items' do
    ...
    context 'set everything!' do
      ...
      it do
        is_expected.to contain_file('/var/www/logs').with(
          'ensure' => 'directory',
          'mode'   => '0600'
        )
      end
      ...
      it do
        is_expected.to contain_concat('30-rspec.example.com.conf').with(
          'owner'   => 'root',
          'mode'    => '0644',
      ...
      it do
        is_expected.to contain_file('30-rspec.example.com.conf symlink').with(
          'ensure' => 'link',
      ...
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/retry=0/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/timeout=5/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/SetEnv force-proxy-request-1.0 1/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/SetEnv proxy-nokeepalive 1/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/noquery interpolate/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/ProxyPreserveHost On/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/ProxyAddHeaders On/)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/ProxyPassReverseCookiePath\s+\/a\s+http:\/\//)
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(/ProxyPassReverseCookieDomain\s+foo\s+http:\/\/foo/)
      end
      ...
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-setenv').with_content(%r{SetEnv FOO=/bin/true})
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-setenv').with_content(%r{SetEnvIf Request_URI "\\.gif\$" object_is_image=gif})
      end
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-setenv').with_content(%r{SetEnvIfNoCase REMOTE_ADDR \^127.0.0.1 localhost=true})
      end
    ...
    context 'proxy_pass_match' do
      ...
      it do
        is_expected.to contain_concat__fragment('rspec.example.com-proxy').with_content(
          /ProxyPassMatch .* http:\/\/backend-a\/ timeout=300/
        ).with_content(/## Proxy rules/)
      end
    end
    ...
    context 'not everything can be set together...' do
      ...
      it do
        is_expected.to contain_file('/tmp/logroot').with(
          'ensure' => 'absent'
        )
      end
      it do
  ...
  describe 'validation' do
    ...
    context 'require unmanaged' do
      ...
      it { is_expected.to contain_concat__fragment('rspec.example.com-directories') }
      it do
        is_expected.to_not contain_concat__fragment('rspec.example.com-directories').with(
          :content => /^\s+Require all granted$/
        )
      end
    end
